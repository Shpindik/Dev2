...
class ClientsTab(tk.Frame):
    def __init__(self, parent, db_connection):
        ...
        self.title_label = tk.Label(self, text="Учет клиентов",
                                    font=("Arial", 16))
        self.title_label.pack(pady=10)
        self.add_client_button = tk.Button(self, text="Добавить клиента",
                                           command=self.show_add_client_form)
        self.add_client_button.pack(pady=10)

        self.show_history_button = tk.Button(
            self,
            text="История покупок клиента",
            command=self.show_client_history
        )
    ...
    def show_add_client_form(self):
        ...
        # Поля для ввода
        tk.Label(add_client_window, text="Имя:").grid(
        ...
        tk.Label(add_client_window, text="Фамилия:").grid(
        ...
        tk.Label(add_client_window, text="Телефон:").grid(
        ...
        tk.Label(add_client_window, text="Email:").grid(
        ...
        tk.Label(add_client_window, text="Дата рождения:").grid(
        ...
        tk.Label(add_client_window, text="Адрес:").grid(
        ...
        save_button = tk.Button(
            add_client_window,
            text="Сохранить",
            command=lambda: self.save_client(add_client_window)
        )
    ...
    def save_client(self, window):
        ...
        cursor = self.db_connection.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM clients WHERE phone = ? OR email = ?",
            (phone, email)
        )
    ...
    def show_client_history(self):
        ...
        if not selected_item:
            messagebox.showwarning(
                "Ошибка", "Пожалуйста, выберите клиента из таблицы."
            )
            return
        ...
        cursor = self.db_connection.cursor()
        cursor.execute("""
            SELECT sales.sale_date, stock.name, sales.quantity, \
                       sales.unit_price, sales.total_price
            FROM sales
        ...
        if not sales:
            messagebox.showinfo(
                "История покупок", "У этого клиента нет покупок."
                )
            return
