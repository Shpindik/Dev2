...
class SalesTab(tk.Frame):
    ...
    def load_products(self, event=None):
        ...
        self.cursor.execute("SELECT id, name, price, quantity FROM stock WHERE branch = ?", (selected_branch,))
        products = self.cursor.fetchall()
        product_names = [product[1] for product in products]
        self.product_combobox['values'] = product_names
        self.products_data = {product[1]: {'id': product[0], 'price': product[2], 'quantity': product[3]} for product in products}
    ...
    def save_sale(self):
        ...
        try:
            ...
            self.cursor.execute('''
                INSERT INTO sales (product_id, client_id, quantity, unit_price, discount, total_price, branch)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (product_data['id'], client_id, quantity, unit_price, discount, total_price, selected_branch))
