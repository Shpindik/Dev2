...
class ReportTab(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Button(
            self,
            text="Отчет по продажам (день)",
            command=lambda: self.generate_sales_report("day")
        ).pack(pady=5)
        ttk.Button(
            self,
            text="Отчет по продажам (неделя)",
            command=lambda: self.generate_sales_report("week")
        ).pack(pady=5)
        ttk.Button(
            self,
            text="Популярные товары",
            command=self.generate_popular_products_report
        ).pack(pady=5)
        ttk.Button(
            self,
    ...
    def generate_sales_report(self, period="day"):
        ...
        query = '''
            SELECT p.name, s.quantity, s.unit_price, s.discount, \
    ...
    def generate_popular_products_report(self):
        ...
        message = "\n".join(
            [
                f"{name}: {total_sales} продаж"
                for name, total_sales in popular_products
            ]
        )
